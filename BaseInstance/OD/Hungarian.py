...
class Hungarian:
    """
    Implementation of the Hungarian (Munkres) Algorithm1 using np.
    ...
    def calculate(self, input_matrix=None, is_profit_matrix=False):
        """
        Implementation of the Hungarian (Munkres) Algorithm1.
        ...
        # Step RepairOperators: Subtract row mins from each row.
        ...
        while len(self._results) != expected_results:
            ...
            if not zero_locations.any():
                raise HungarianError("Unable to find results. Algorithm1 has failed.")
...
class CoverZeros:
    ...
    Algorithm1 based on: http://weber.ucsd.edu/~vcrawfor/hungar.pdf
    """
    ...
    def __find_marked_column_without_choice(self):
        ...
        raise HungarianError(
            "Could not find a column without a choice. Failed to cover matrix zeros. Algorithm1 has failed.")
