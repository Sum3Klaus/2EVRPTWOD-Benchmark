    ...
    for filename in os.listdir(folder_path):
        ...
        if files_read < 100:
            ...
            file_name = f'{datetime.date.today().strftime("%Y-%m-%d")}' + file_path.split('\\')[
                -1] + f'_GPTV_set3_ALNS_3407_3600' + '.xlsx'
        ...
        with open(new_benchmark_file, 'a', encoding='utf-8') as dest:
            ...
            for i in alg.ins.graph.customer_list:
                cus_id = i
                cus_ready_time = alg.ins.graph.vertex_dict[i].ready_time
                cus_due_time = alg.ins.graph.vertex_dict[i].due_time
                dest.write(f'({cus_id},{cus_ready_time},{cus_due_time}), ')
            dest.write('\n!----------------------------------------------------------------\n')
            dest.write(f'!OD capacity: {alg.ins.model_para.vehicle_od_capacity} \n')

            dest.write(
                '!Task id, origin id, x_coord, y_coord, ready time, due time, terminate id, x_coord, y_coord, ready time, due time \n')
            for task in alg.ins.graph.od_task_dict:
                task_id = task
                # origin = alg.ins.graph.od_task_dict[task_id].origin_node
                # terminate = alg.ins.graph.od_task_dict[task_id].terminate_node
                o_id = alg.ins.graph.od_task_dict[task_id].origin_node.id_
                o_x = alg.ins.graph.od_task_dict[task_id].origin_node.x_coord
                o_y = alg.ins.graph.od_task_dict[task_id].origin_node.y_coord
                o_ready_time = alg.ins.graph.od_task_dict[task_id].origin_node.ready_time
                o_due_time = alg.ins.graph.od_task_dict[task_id].origin_node.due_time

                d_id = alg.ins.graph.od_task_dict[task_id].terminate_node.id_
                d_x = alg.ins.graph.od_task_dict[task_id].terminate_node.x_coord
                d_y = alg.ins.graph.od_task_dict[task_id].terminate_node.y_coord
                d_ready_time = alg.ins.graph.od_task_dict[task_id].terminate_node.ready_time
                d_due_time = alg.ins.graph.od_task_dict[task_id].terminate_node.due_time
