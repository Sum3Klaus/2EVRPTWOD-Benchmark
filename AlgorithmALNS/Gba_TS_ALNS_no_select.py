...
class GbaTsAlns(object):
    ...
    def __init__(self,
        ...
        if is_dual:
            ...
            self.destroyList = [RandomDestroy, ShawDestroy, WorseDestroy, WaitingDestroy, HistoryBasedDestroy,
                                ConflictDestroy, HistoricalArriveDestroy]
            self.repairList = [GreedyRepair, RandomRepair, RegretRepair, DistanceRepair, SlackTimeRepair]
        ...
        self.gda = GreatDelugeAlgorithm.GreatDelugeAlgorithm(alns=self,
    ...
    def gen_new_route_no_select(self):
        new_route = Route()
        sate = self.ins.graph.vertex_dict[list(self.depots_dict.keys())[0]]
        new_route.set_sate_route(sate=sate)

        new_route.arriveTime[0] = new_route.timeTable[0] = self.ins.graph.vertex_dict[
            list(self.depots_dict.keys())[0]].ready_time
    ...
    def split_route(self,
        ...
        depot_start = list(self.depots_dict.keys())[0]
        depot_return = self.depots_dict[depot_start]

        ver_index = 0
        sol = Sol(ins=self.ins)
        ver_assigned = []

        new_route = self.gen_new_route_no_select()
    ...
    def run(self):
        ...
        print('*' * 50, f'sate NO.{self.depots_dict}', '*' * 50)
        # sol = deepcopy(self.best_sol)
        sol = self.best_sol
